...
- **Qwen3-TTS code-predictor sampling runs on the GPU** (default; `QWEN3_TTS_CPU_SAMPLE=1` reverts to per-step CPU sampling). The 15 codebook steps each need the sampled token to build the next step's embedding, which naively forced a blocking `to_vec2` readback per step (15 syncs/frame). `gpu_sample_token` does temperature + Gumbel-max argmax as on-device tensor ops (top-k/top-p skipped — the defaults top_k=50/top_p=1.0 only trim the negligible tail) with ONE readback per frame. Measured on an idle machine it cuts the predictor ~156→112 ms/frame (~22% end-to-end on the standard sentence). **It does not break RTF < 1**: the residual cost is the 15 sequential 5-layer predictor forwards (~75 tiny kernel launches/frame, ~3 ms/layer regardless of hidden size 1024 vs 2048 → pure launch latency, not FLOPS — which is also why 0.6B ≈ 1.7B in RTF). The real lever would be CUDA-graph-style kernel capture, which candle 0.11 doesn't expose on Metal. Correctness guards: `tests/gpu_sampling.rs` (greedy==CPU argmax; Gumbel-max is a valid categorical draw) + ASR round-trip. Profile per-stage with `QWEN3_TTS_PROF=1`. **Benchmark caveat**: RTF numbers are only meaningful on an idle machine — a single competing GPU/CPU process makes identical runs swing 2–6×; check `sysctl -n vm.loadavg` (~<3) before trusting a number.
