...
- `tts cosyvoice3` — Fun-CosyVoice3-0.5B (Qwen2-0.5B LM + DiT-CFM + HiFT, 24kHz; ported from CrispASR C++/ggml). 8 baked voices (`--voice`), zero-shot cloning (`--ref` + `--ref-text`, required). Prefers `cosyvoice3-llm-q4_k.gguf` (QMatMul) / `cosyvoice3-flow-q8_0.gguf` (F16) over `llm.pt`/`flow.pt` when present.
