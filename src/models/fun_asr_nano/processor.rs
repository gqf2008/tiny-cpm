//! Ported from aha (github.com/jhqxxx/aha) src/models/fun_asr_nano/processor.rs
//!
//! Adapted: aha's `process_info` took a `ChatCompletionParameters` (OpenAI-style
//! request) and pulled the user text + audio URL out of it. tiny-cpm has no
//! request types, so `process_info` takes the user text and an already-loaded,
//! resampled audio tensor (see `crate::utils::audio_utils::load_audio_with_resample`)
//! directly. Everything else is verbatim.
use anyhow::Result;
