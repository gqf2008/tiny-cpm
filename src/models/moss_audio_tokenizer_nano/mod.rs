//! Ported from aha (github.com/jhqxxx/aha) src/models/moss_audio_tokenizer_nano/mod.rs
pub mod config;
...
use crate::{
    ...
    models::moss_audio_tokenizer_nano::config::{
        MossAudioTokenizerConfig, MossAudioTokenizerModuleConfig, MossAudioTokenizerQuantizerKwargs,
    },
...
impl MossAudioTokenizer {
    ...
    pub fn new(
        ...
        for (layer_id, cfg) in config.decoder_kwargs.iter().enumerate() {
            // Python reference (modeling_moss_audio_tokenizer.py:2401-2414): the
            // running frame rate is multiplied back by each decoder module's
            // downsample_ratio, mirroring the encoder loop.
            let downsample_ratio = if cfg.module_type == "PatchedPretransform"
                && let Some(patch_size) = cfg.patch_size
            {
