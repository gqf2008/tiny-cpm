...
- `tts cosyvoice3` — Fun-CosyVoice3-0.5B (Qwen2-0.5B AR speech-token LM + DiT-CFM flow + HiFT vocoder, 24kHz; ported from [CrispASR](https://github.com/CrispStrobe/CrispASR)'s C++/ggml implementation). 8 baked voices via `--voice`, zero-shot cloning via `--ref` + `--ref-text` (required), uses s3tok/CAMPPlus GGUF encoders.
...
cargo run --release -- tts cosyvoice3 <model-dir> "<text>" <out.wav> [--voice name] [--ref ref.wav --ref-text "<text>"] [--steps 6]
